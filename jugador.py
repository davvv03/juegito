...
class Fondo(pygame.sprite.Sprite):
    def __init__(self,posicion,imagen,delay):
        super(Fondo,self).__init__()
        self.images = itertools.cycle(imagen)
        self.actual=next(self.images)
        self.rect=pygame.Rect(posicion,self.image.get_rect().size)
...
def cargargif(path):
    images=[pygame.image.load(path).convert() for file_name in sorted (os.listdir(path))]
    return  images  
